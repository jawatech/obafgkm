        ...
        files.forEach(function (file) {
          ...
          if (stat) {
            if (stat.isDirectory()) {
              ...
              href = viewerurl + "?file=" + encodeURIComponent(item);
              label = file;
              extraAttributes = ' target="pdf"';
            } else if (path.extname(file).toLowerCase() === ".epub") {
              href = viewerurl2 + "?bookPath=" + encodeURIComponent(item);
              label = file;
              extraAttributes = ' target="pdf"';
            } else if (all) {
              href = encodeURI(item);
